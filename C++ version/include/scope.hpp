...
inline Color disturbColorInScope(const Color &color, const Scope &scope, const double *range)
{
    ...
    c = randomDouble(std::max(c - range[1], scope.chroma_scope[0] + scope.margin), std::min(c + range[1] + scope.margin, scope.chroma_scope[1]));
    l = randomDouble(std::max(l - range[2], scope.lumi_scope[0] + scope.margin), std::min(l + range[2] + scope.margin, scope.lumi_scope[1]));
    if (!scope.all_hue)
    {
